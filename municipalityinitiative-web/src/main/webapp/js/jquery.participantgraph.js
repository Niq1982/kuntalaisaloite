...
(function ($) {
  ...
  var MIN_GRAPH_DAYS = 30,
    DATE_FORMAT = 'YYYY-MM-DD',
    DATE_FORMAT_LOCAL = 'D.M.YYYY',
    DATE_FORMAT_LOCAL_DMY = 'D.M.YY',
    DATE_FORMAT_LOCAL_DM = 'D.M.';
  ...
  var setEndDate = function (start, end, minDays) {
    ...
    if (end) {
      if (moment(mEnd).diff(mStart, 'days') < minDays) {
        return moment(mStart).add('days', addDays).format(DATE_FORMAT);
      } else {
        return end;
      }
    } else {
  ...
  var methods = {
    init : function (options) {
      ...
      // Not ended - less than 30 days
      // testStart = moment().subtract('days', 10).format(DATE_FORMAT);
      // testEnd = null;

      // Not ended - more than 30 days
      // testStart = moment().subtract('days', 60).format(DATE_FORMAT);
      // testEnd = null;

      // Not ended - one year
      // testStart = moment().subtract('days', 365).format(DATE_FORMAT);
      // testEnd = null;
  ...
  $.fn.participantGraph = function (methodOrOptions) {
    if ( methods[methodOrOptions] ) {
      ...
      $.error( 'Method ' +  methodOrOptions + ' does not exist on jQuery.participantGraph' );
    }
  };
  ...
  function iterateData(data) {
    ...
    for (i = 0; i < days; i++) {
      ...
      labels.push(curDate.format(DATE_FORMAT_LOCAL));
      rawData.push(value);
  ...
  function testDataGererator(firstDate, lastDate, daily, tolerance) {
    ...
    for (i = 0; i < days; i++) {
      ...
      data.push({
        d: moment(curDate).format(DATE_FORMAT),
        n: rnd > 0 ? rnd : 0
      });
      curDate = moment(curDate).add('days', 1);
    }
  ...
  initGraph = function (element, settings) {
    ...
    var width = settings.width,
      ...
      colorToolTip = settings.colorToolTip,
      ...
      dateFormat = settings.width < 500 ? (settings.width < 400 ? DATE_FORMAT_LOCAL_DM : DATE_FORMAT_LOCAL_DMY) : DATE_FORMAT_LOCAL,
    ...
    // NOTE: enable when max count is available
    // if (settings.cumulative) {
    ...
    var frame = r.popup(100, 100, label, 'right').attr({fill: colorToolTip, 'stroke-width': 0, 'fill-opacity': 1}).hide();
