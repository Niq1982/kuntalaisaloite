...
public class InitiativeController extends BaseController {
    ...
    @Resource StatusService statusService;
...
//    }
    ...
    @RequestMapping(value={ VIEW_FI, VIEW_SV }, method=GET, params=PARAM_INVITATION_CODE)
    public String respondToInvitationGet(@PathVariable("id") Long initiativeId, @RequestParam(PARAM_INVITATION_CODE) String invitationCode, Locale locale, HttpServletRequest request) {
        Urls urls = Urls.get(locale);
        InitiativePublic initiative = initiativeService.getInitiativeForPublic(initiativeId);
        if (initiative.getState() == InitiativeState.PROPOSAL) {    //accepting invitations is allowed only in PROPOSAL state
            ...
            return redirectWithMessage(urls.view(initiativeId), RequestMessage.ACCEPTING_INVITATIONS_NOT_ALLOWED, request);
        }
    ...
    @RequestMapping(value={ VIEW_FI, VIEW_SV }, method=POST, params=ACTION_DECLINE_INVITATION)
    public String declineInvitationPost(@PathVariable("id") Long initiativeId, @RequestParam(PARAM_INVITATION_CODE) String invitationCode, Model model, Locale locale, HttpServletRequest request) {
        Urls urls = Urls.get(locale);

        clearSessionInvitationCode(initiativeId, request);

        if (initiativeService.declineInvitation(initiativeId, invitationCode)) {
            return redirectWithMessage(urls.view(initiativeId), RequestMessage.DECLINE_INVITATION, request);
        } else {
            return redirectWithMessage(urls.view(initiativeId), RequestMessage.OPEN_INVITATION_FAILED, request);
        }
    }
    
    @RequestMapping(value={ VIEW_FI, VIEW_SV }, method=GET, params=ACTION_ACCEPT_INVITATION)
    public String acceptInvitationGet(@PathVariable("id") Long initiativeId, Model model, Locale locale, HttpServletRequest request) {
        ...
        return acceptInvitationView(initiativeId, invitation, author, model);
    }
    
    @RequestMapping(value={ VIEW_FI, VIEW_SV }, method=POST, params=ACTION_ACCEPT_INVITATION)
    public String acceptInvitationPost(@PathVariable("id") Long initiativeId, @RequestParam(PARAM_INVITATION_CODE) String invitationCode, @ModelAttribute("currentAuthor") Author author, BindingResult bindingResult, Model model, Locale locale, HttpServletRequest request) {
        Urls urls = Urls.get(locale);
        model.addAttribute(ALT_URI_ATTR, urls.alt().view(initiativeId));
    ...
    public @ResponseBody List<InitiativeInfo> jsonList(@RequestParam(value = JSON_OFFSET, required = false) Integer offset,
        ...
        InitiativeSearch search = new InitiativeSearch(true, false);
        search.setLimit(Math.min(MAX_INITIATIVE_JSON_RESULT_COUNT, limit));
        search.setOrderBy(InitiativeSearch.OrderBy.ID);
        if (offset != null) {
            search.setOffsetIndex(offset);
        }
        return initiativeService.findInitiatives(search);
    }
    ...
    public @ResponseBody JsonpObject<List<InitiativeInfo>> jsonpList(@RequestParam(JSONP_CALLBACK) String callback,
                                                                     ...
                                                                     @RequestParam(value = JSON_LIMIT, required = false) Integer limit) {
        return new JsonpObject<List<InitiativeInfo>>(callback, jsonList(offset, limit));
    }
    ...
    public void initBinder(WebDataBinder binder, Locale locale) {
        ...
        // binder.registerCustomEditor(InitURI.class, new InitURIEditor());
    }
    ...
    private String acceptInvitationView(Long initiativeId, Invitation invitation, Author author, Model model) {
        model.addAttribute("invitation", invitation);
        model.addAttribute("currentAuthor", author);
        model.addAttribute("initiative", initiativeService.getInitiativeForPublic(initiativeId));
