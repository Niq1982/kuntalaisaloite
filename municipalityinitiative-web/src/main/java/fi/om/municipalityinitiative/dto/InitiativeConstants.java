package fi.om.municipalityinitiative.dto;

public class InitiativeConstants {
    public static final int INITIATIVE_NAME_MAX = 512;
    public static final int INITIATIVE_PROPOSAL_MAX = 100000;
    public static final int INITIATIVE_EXTRA_INFO_MAX = 4096;
    public static final int INITIATIVE_COMMENT_MAX = 1024;
    public static final int CONTACT_NAME_MAX = 100;
    public static final int CONTACT_EMAIL_MAX = 100;
    public static final int CONTACT_PHONE_MAX = 30;
    public static final int CONTACT_ADDRESS_MAX = 256;
    public static final int AUTHOR_MESSAGE_MAX = 2048;
    public static final int ATTACHMENT_DESCRIPTION_MAX = 100;
    public static final int MUNICIPALITY_DECISION_MAX = 2000;
    public static final int MUNICIPALATY_DESCRIPTION_MAX = 10000;
    public static final int VIDEO_MAX_LENGTH = 100;
}
