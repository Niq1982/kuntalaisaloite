...
public class InitiativeDaoImpl implements InitiativeDao {
    ...
    public InitiativeInfo lockInitiativeForUpdate(Long id) {
        return queryFactory
        .from(qInitiative)
        .where(qInitiative.id.eq(id))
        .forUpdate()
        .singleResult(initiativeInfoMapping);
    }
    ...
    @Override
    @Transactional(readOnly=true)
    public List<InitiativeInfo> findInitiatives(InitiativeSearch search, Long userId, int minSupportCount) {
        PostgresQuery qry = queryFactory.from(qInitiative);
        BooleanBuilder where = new BooleanBuilder();

        if (search.isIncludePublic()) {
            where.or(
                    qInitiative.state.in(InitiativeState.ACCEPTED, InitiativeState.DONE)
                    .and(qInitiative.supportcount.add(qInitiative.externalsupportcount).goe(minSupportCount))
                );
        }
        
        if (search.isIncludeOwn() && userId != null) {
        ...
        if (search.getOffsetIndex().isPresent()) {
            qry.offset(search.getOffsetIndex().get());
        }
        if (search.getLimit().isPresent()) {
            qry.limit(search.getLimit().get());
        }
        
        if (!search.isIncludePublic() && !search.isIncludeOwn()) {
            return Lists.newArrayList(); // empty result
        } 
        else {
            qry.where(where);
            qry.orderBy(createOrderBy(search.getOrderBy()));

            Map<Long, InitiativeInfo> initiativesById = qry.map(qInitiative.id, initiativeInfoMapping);
            
            return Lists.newArrayList(initiativesById.values());
        }
    }

    private static OrderSpecifier<?> createOrderBy(InitiativeSearch.OrderBy orderBy) {
        switch (orderBy) {
            case START_DATE:
...
//    private void populateInvitation(StoreClause<?> store, Invitation invitation) {
//        store.set(qInvitation.sent, invitation.getSent());
//        store.set(qInvitation.invitationcode, invitation.getInvitationCode());
//        store.set(qInvitation.initiator, (invitation.getRole() == AuthorRole.INITIATOR));
//        store.set(qInvitation.role, role2long(invitation.getRole()));
//        store.set(qInvitation.email, invitation.getEmail());
//    }
